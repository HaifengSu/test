...
bool Actuation::GetControl(hal_quadrotor::State &state, double t, hal_quadrotor::Control &control)
{
	// Find the controller
	Controller* ptr;
	switch (current)
	{
		case CONTROLLER_ANGLESHEIGHT: 	ptr = (Controller*) &cAnglesHeight;		state.controller="AnglesHeight";	break;
		case CONTROLLER_EMERGENCY: 		ptr = (Controller*) &cEmergency;		state.controller="Emergency";		break;
		case CONTROLLER_HOVER: 			ptr = (Controller*) &cHover;			state.controller="Hover";			break;
		case CONTROLLER_IDLE: 			ptr = (Controller*) &cIdle;				state.controller="Idle";			break;
		case CONTROLLER_LAND: 			ptr = (Controller*) &cLand;				state.controller="Land";			break;
		case CONTROLLER_TAKEOFF: 		ptr = (Controller*) &cTakeoff;			state.controller="Takeoff";			break;
		case CONTROLLER_VELOCITYHEIGHT: ptr = (Controller*) &cVelocityHeight;	state.controller="VelocityHeight";	break;
		case CONTROLLER_VELOCITY: 		ptr = (Controller*) &cVelocity;			state.controller="Velocity";		break;
		case CONTROLLER_WAYPOINT: 		ptr = (Controller*) &cWaypoint;			state.controller="Waypoint";		break;
		default:		
			return false;
	}
	...
	if (ptr->HasGoalBeenReached())
	{
		...
		case CONTROLLER_LAND:
			if (DEBUG) ROS_INFO("Goal reached. Switching to idle.");
			Switch(CONTROLLER_IDLE);
	...
	ptr->GetControl(state,t,control);
