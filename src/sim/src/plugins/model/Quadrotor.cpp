	...
	class Quadrotor : 
		...
		public hal::sensor::Orientation 	/* Exposes Orientation sensor    */
 	{
  	...
  	private:
		...
		// Animate motors
		void AnimateMotor(const char *name, const char *motor, double rpm)
		{
			std::map<std::string, common::NumericAnimationPtr> anim;
			anim[motor].reset(new common::NumericAnimation(name, 1.0, true));
			common::NumericKeyFrame *key;
			key = anim[motor]->CreateKeyFrame(0.0);
			key->SetValue(rpm*2*3.1415*0/4);
			key = anim[motor]->CreateKeyFrame(0.25);
			key->SetValue(rpm*2*3.1415*1/4);
			key = anim[motor]->CreateKeyFrame(0.50);
			key->SetValue(rpm*2*3.1415*2/4);
			key = anim[motor]->CreateKeyFrame(0.75);
			key->SetValue(rpm*2*3.1415*3/4);
			key = anim[motor]->CreateKeyFrame(1.0);
			key->SetValue(rpm*2*3.1415*4/4);
			modelPtr->SetJointAnimation(anim);
		}
  	...
  	public:
	    ...
	    void Load(physics::ModelPtr model, sdf::ElementPtr root)
	    {
	    	...
	    	// Het
	    	std::string halName = (std::string)"/hal/quadrotor/hummingbird/" + model->GetName();
			...
			// Create blade animation (10rpm)
			AnimateMotor("animation_motor0","motor0",5);
			AnimateMotor("animation_motor1","motor1",5);
			AnimateMotor("animation_motor2","motor2",5);
			AnimateMotor("animation_motor3","motor3",5);
	    }
